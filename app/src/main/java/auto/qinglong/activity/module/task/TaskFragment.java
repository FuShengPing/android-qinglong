...
import auto.qinglong.activity.module.log.LogActivity;
...
public class TaskFragment extends BaseFragment {
    ...
    public void init() {
        ...
        taskAdapter.setTaskInterface(new ItemInterface() {
            ...
            public void onLog(QLTask QLTask) {
                ...
                intent.putExtra(LogActivity.ExtraName, QLTask.getName());
                intent.putExtra(LogActivity.ExtraPath, QLTask.getLogPath());
    ...
    public static List<QLTask> sortTasks(@NonNull List<QLTask> fromData) {
        List<QLTask> runningData = new ArrayList<>();
        List<QLTask> pinnedData = new ArrayList<>();
        List<QLTask> freeData = new ArrayList<>();
        List<QLTask> limitData = new ArrayList<>();

        for (QLTask QLTask : fromData) {
            if (QLTask.getTaskState() == TaskState.RUNNING) {
                runningData.add(QLTask);
            } else if (QLTask.getIsPinned() == 1) {
                pinnedData.add(QLTask);
            } else if (QLTask.getTaskState() == TaskState.FREE) {
                freeData.add(QLTask);
            } else {
                limitData.add(QLTask);
            }
        }
        fromData.clear();
